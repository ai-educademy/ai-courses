...
Treats the input as a raw byte stream - no pre-tokenisation by spaces. This is crucial for languages like Japanese and Chinese that do not use spaces between words. Many GPT and Llama-family models use SentencePiece-style or byte-level approaches.

## How GPT-Style Models Tokenise Text

OpenAI GPT-style models use BPE variants with large token vocabularies. Some surprising behaviours:
...
Every API call to a frontier model such as GPT-6 Astra, Claude Opus 5.5, or Gemini 3.1 Pro is billed per token. Understanding tokenisation helps you:

- **Estimate costs** before running large jobs.
- **Optimise prompts** - shorter prompts with the same meaning save money.
- **Respect context windows** - current frontier models support anything from hundreds of thousands to over a million tokens, but exceeding the selected model's limit still truncates or rejects your input.
...
<FunFact>
OpenAI's open-source `tiktoken` library lets you tokenise text locally with the same family of algorithms used by OpenAI GPT models. Try it on your own writing to see how many tokens your messages really cost.
</FunFact>
