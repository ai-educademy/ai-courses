...
<FunFact>
Frontier LLMs use token vocabularies that commonly run to around 100,000 tokens or more. The entire English language has roughly 170,000 words in common use, but an LLM's token vocabulary also covers many languages, code syntax, emoji, and specialised terminology simultaneously.
</FunFact>
...
Early GPT models had context windows of 2,048 tokens (about 1,500 words). Modern flagship models have pushed that into the hundreds of thousands, and the largest now advertise a million tokens or more, easily enough to hold an entire novel or a large codebase at once.
...
- **Frontier closed models** usually keep training details secret, but estimates suggest trillions of tokens
- **Llama 4 Scout** was trained on about 40 trillion tokens of multimodal data
...
Training compute costs are enormous: GPT-3's training cost an estimated $4.6 million in compute alone. Current frontier model training can cost hundreds of millions when data, GPUs, energy, and engineering are included.
...
GPT-2 (2019) could barely write coherent paragraphs. GPT-3 (2020) could write essays, solve logic puzzles, and translate languages it had barely seen. Later frontier models can pass difficult medical, legal, and coding benchmarks.
