...
Traite l’entrée comme un flux brut d’octets - sans pré-tokenisation par les espaces. C’est crucial pour les langues comme le japonais et le chinois qui n’utilisent pas d’espaces entre les mots. De nombreux modèles des familles GPT et Llama utilisent des approches de type SentencePiece ou au niveau des octets.

## Comment les modèles de style GPT tokenisent le texte

Les modèles de style GPT d’OpenAI utilisent des variantes de BPE avec de grands vocabulaires de tokens. Quelques comportements surprenants :
...
Chaque appel API à un modèle de pointe comme GPT-6 Astra, Claude Opus 5.5 ou Gemini 3.1 Pro est facturé au token. Comprendre la tokenisation vous aide à :
...
- **Respecter les fenêtres de contexte** - les modèles de pointe actuels prennent en charge de quelques centaines de milliers à plus d’un million de tokens, mais dépasser la limite du modèle choisi tronque ou rejette toujours votre entrée.
...
<FunFact>
La bibliothèque open source `tiktoken` d’OpenAI vous permet de tokeniser du texte localement avec la même famille d’algorithmes que celle utilisée par les modèles GPT d’OpenAI. Essayez-la sur vos propres écrits pour voir combien de tokens vos messages coûtent réellement.
</FunFact>
